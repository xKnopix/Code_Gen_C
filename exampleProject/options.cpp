...
namespace DHBW_Programmieren
{
    ...
    void Beispiel_XML::printHelp(int signPerLine)
    {
        ...
        myPrint("\n", 79);
        myPrint("Author Name: Michael Hornstein, Jacob Hochbaum, Johannes Muessig, Jakob Irle, Linus Fallert, ", 79);
    ...
    void Beispiel_XML::parseOptions(int argc, char *argv[])
    {
        ...
        for (int i = 1; i < argc; i++)
        {
            ...
            if (string(argv[i]) == "-h" || string(argv[i]) == "--help")
            {
                for (int r = 0; r < exclusionValuesSorted.size(); r++)
                {
                    ...
                    for (int q = 0; q < temp.size(); q++)
                    {
                        ...
                        {
                            cout << "--help ist nicht erlaubt mit " + argumentnames[r] << endl;
                            exit(EXIT_FAILURE);
                        }
                    }
                }
            ...
            if (string(argv[i]) == "--parse-only")
            {
                ...
                {
                    vector<string> tempExclusionVec = exclusionValuesSorted[j];
                    for (int k = 0; k < tempExclusionVec.size(); k++)
                    {
