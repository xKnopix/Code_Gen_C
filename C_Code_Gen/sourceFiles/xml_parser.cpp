...
{ // return einen Wert von einem Attribut, welcher in einem Node gesucht wird
    string ret;

    if (Node->hasAttributes())
    {                                                     // Schaue ob Author von (i) Attribute hat wie z.B Name, Email, Phone
    ...
    {
        cout << "Es gibt keine Attribute" << endl;
    }
    return ret;
}

DOMDocument *xml_parser::GetDomDocument(std::string xmlName)
{ // gibt ein dom document zurück, mit dem dann weiter gearbeitet werden kann
    try
    {
